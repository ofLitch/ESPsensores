 ...
 */
void taskMQ135(void *pvParameters) {    // Desempaquetar los parámetros
    void **params = (void **)pvParameters;
    SemaphoreHandle_t mutex = (SemaphoreHandle_t)params[0];
    unsigned short int  mq135Pin = *((unsigned short int  *)params[1]);

    // Inicializar el sensor MQ135
    MQUnifiedsensor mqSensor ("ESP32", MQ135_VOLTAGE, MQ135_ADC_RESOLUTION, mq135Pin, "MQ-135");
